...
const (
	binaryPrefix           = "a2a-"
	transportExcludePrefix = "a2a-transport-"
)

// Discovered describes a command plugin binary found on PATH.
type Discovered struct {
	// Name is the command name (the binary suffix after "a2a-").
	Name string
	// Path is the absolute path to the plugin binary.
	Path string
	// Info holds the plugin's self-reported metadata. It is nil when the plugin
	// does not implement the "info" subcommand or could not be queried.
	Info *cliplugin.Info
...
func QueryInfo(ctx context.Context, binary string) (*cliplugin.Info, error) {
	ctx, cancel := context.WithTimeout(ctx, 3*time.Second)
	defer cancel()

	cmd := exec.CommandContext(ctx, binary, cliplugin.SubcommandInfo)
	out, err := cmd.Output()
	if err != nil {
		return nil, fmt.Errorf("running %q %s: %w", binary, cliplugin.SubcommandInfo, err)
	}
	var info cliplugin.Info
...
func discover() []Discovered {
	...
	for _, dir := range pathDirs {
		...
		for _, entry := range entries {
			...
			if strings.HasPrefix(base, transportExcludePrefix) {
				continue
			}
